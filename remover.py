...
class KoreaExpresswayPIIRemover:
    ...
    def __init__(self, use_llm=False, llm_model="qwen3:8b"):
        """PII 제거 엔진 초기화"""
        ...
        # LLM 엔진 초기화 (개발환경에서는 비활성화)
        self.llm_endpoint = None
        if self.use_llm:
            try:
                # 환경변수에서 LLM 엔드포인트 확인
                import os
                vllm_endpoint = os.getenv('VLLM_ENDPOINT')  # 예: http://internal-gpu-server:8000
                ollama_endpoint = os.getenv('OLLAMA_ENDPOINT', 'http://localhost:11434')
                
                if vllm_endpoint:
                    print(f"[설정] VLLM 엔드포인트 사용: {vllm_endpoint}")
                    self.llm_endpoint = vllm_endpoint
                    self.llm_type = "vllm"
                else:
                    print(f"[설정] Ollama 엔드포인트 사용: {ollama_endpoint}")
                    self.llm_endpoint = ollama_endpoint
                    self.llm_type = "ollama"
                
                self.llm_remover = self._create_llm_engine(llm_model)
                self.llm_remover.parent = self  # 부모 참조 추가
            ...
            def process_text_batch(self, texts):
                ...
                for i, text in enumerate(texts):
                    if len(str(text).strip()) >= 5:
                        valid_texts.append((i, str(text).strip()[:500]))  # 길이 제한
                ...
                prompt = f"""다음 텍스트들에서 개인정보 찾기. 각 번호별로 JSON 응답.
...
JSON: {{"0":[{{"type":"PERSON","text":"이름"}}], "1":[]}}"""

                try:
                    if hasattr(self, 'parent') and hasattr(self.parent, 'llm_type') and self.parent.llm_type == "vllm":
                        # VLLM API 호출
                ...
                results = []
                for text in texts:
                    processed, entities = self._regex_fallback(str(text))
                    results.append((processed, entities))
                return results
            
            def process_text(self, text):
                # 배치 처리로 변경된 메소드
                results = self.process_text_batch([text])
            ...
            def _regex_fallback(self, text):
                ...
                # 이름 + 님
                nim_matches = re.findall(r'([가-힣]{2,4})님', text)
                for name in nim_matches:
                    full_match = f"{name}님"
                    entities.append(PIIEntity("PERSON", full_match, "[이름]", 0.9))
                    result = result.replace(full_match, "[이름]")
    ...
    def _setup_korean_recognizers(self):
        ...
        korean_name_recognizer = PatternRecognizer(
            ...
            patterns=[
                ...
                # "님" 붙은 이름 패턴 (높은 신뢰도) - 이지헌님, 장종외님 등
                Pattern(
                    name="korean_name_with_nim",
                    regex=r"([가-힣]{2,4})님",
                    score=0.95,
                ),
            ],
            name="korean_name_recognizer",
            context=["이름", "성명", "저는", "제", "연락처", "신고", "문의", "담당자", "님", "감사"],
        )
    ...
    def _advanced_multi_pass_processing(self, text):
        ...
        for pass_num in range(max_passes):
            ...
            # 4단계: 간단한 이름 패턴 우선 처리 (직접 치환)
            # "님" 붙은 이름들 (띄어쓰기 포함)
            current_text = re.sub(r'([가-힣]{2,4})\s*님', '[이름]', current_text)
            
            # 직급과 함께 나오는 이름 (선임, 대리 등)
            current_text = re.sub(r'([가-힣]{2,4})\s+(선임|대리|주임|사원|과장|차장|부장|팀장|실장|소장)', '[담당자]', current_text)

            # 5단계: 복잡한 개인 이름 패턴 (보호 단어 제외 + 더 엄격한 조건)
            personal_patterns = [
    ...
    def process_text(self, text):
        ...
        # 2단계: LLM 보조 처리 (선택적, 짧은 텍스트만)
        if self.use_llm and self.llm_remover and len(str(text)) < 100:
            try:
                anonymized, entities = self.llm_remover.process_text(text)
                if entities:  # LLM이 추가 PII를 탐지했다면
                    progress_tracker.add_pii_removed(len(entities))
                    # 정규식 결과와 LLM 결과 결합
                    processed_text = anonymized if len(entities) > 0 else processed_text
            except Exception as e:
                print(f"[경고] LLM 보조 처리 실패: {e}")
...
def upload_file():
    ...
    try:
        ...
        try:
            ...
            final_file_path = os.path.join(downloads_path, final_filename)
            shutil.copy2(output_file, final_file_path)
            print(f"[완료] 파일을 다운로드 폴더에 저장: {final_file_path}")
        except Exception as e:
            print(f"[경고] 다운로드 폴더 복사 실패: {e}")
    ...
    try:
        ...
        if os.path.exists(file_path):
            ...
            if os.path.exists(current_path):
                ...
                return "파일을 찾을 수 없습니다. 이미 다운로드되었거나 만료되었을 수 있습니다.", 404
    except Exception as e:
